"""
Triton kernels for the shortwave Scattering module optimized for
inference in evaluate_network_fast.py for the NVDIA T4 GPU. 

Contains several alternative kernels, mostly for scattering, each with a different computational strategy. Currently launch_optical_scattering_bd_v2() gives the best performance for scattering on the T4

Some of these kernels are wired into evaluate_network_fast and can be invoked by setting the appropriate environmental variables.

Others are not used now, but are kept
for reference purposes.

Notes in each kernel indicate its current status.
...
# ---------------------------------------------------------------------------
# Kernel for processing direct radiation only
#
# Not currently used
# ---------------------------------------------------------------------------
@triton.autotune(
...
#--------------------------------------------------------------------------
# Kernel for processing diffuse radiation only
#
# Not currently used
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Kernel combining direct + diffuse radiation.
#
# Not currently used
#
...
# Not currently used
#
# Computes tau in addition to scattering (does not do complete
...
# Fused ke MLP kernel.
#
# Fully fused optical depth kernel (no scattering computation)
#
# Not currently used
#
...
# ---------------------------------------------------------------------------
# T4-optimized direct and diffuse kernels with FUSED ke computation.
#
# Called by launch_optical_scattering_fke()
#
# Identical to _optical_split_{direct,diffuse}_T4_kernel 
# except the optical depth (ke)
# values are computed inline from temperature/pressure 
# instead of loaded
# from a pre-computed [B, 6] tensor.  
# This eliminates the separate ke
# kernel launch and the ke DRAM round-trip.
#
# Corresponds to USE_FKE_SCATTERING in evaluate_network_fast.py
# ---------------------------------------------------------------------------
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_B': 64},  num_warps=2),
...
#
# Does not take advantage of block diagonal weight matrix
# for the MLPs. 
#
# NOT CURRENTLY USED
# ---------------------------------------------------------------------------
...
# I'm not sure what the next kernel tries to do
# It looks like an early attempt to exploit
# the block diagonal weight matrix for the direct
# and diffuse MLPs.
# And, it seems to do so, without extra
# split and transpose operations.
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_B': 64},  num_warps=2),
        triton.Config({'BLOCK_B': 128}, num_warps=4),
...
# ---------------------------------------------------------------------------
# T4-optimized kernels for direct and diffuse paths with pre-stored FP16 weights.
#
# Does not take advantage of block diagonal weight matrix
...
#
# Runs if none of the USE_BD_* environmental 
# variables is set in evaluate_network_fast.py
# ---------------------------------------------------------------------------
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_B': 64},  num_warps=2),
...
# 
# Does unnecessary and inefficient split, 
...
# 
# Corresponds to USE_BD_SCATTERING in evaluate_network_fast()
# # ---------------------------------------------------------------------------
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_B': 64},  num_warps=2),
...
# Separate "split" kernels for direct and diffuse
# radiation that take advantage of the the 
# block diagonal weight matrices without 
# unnecessarysplit, transpose, and join operations.
#
# Currently, the default used in evaluate_network_fast.py.
#
# Achieves the best performance so far for the
# T4 GPU
#
...
#
# Total of 4 kernels: direct and diffuse paths
# each split into two kernels for the their
...
# 
# Corresponds to USE_BD_V2_SPLIT_SCATTERING in
# evaluate_network_fast.py
# ---------------------------------------------------------------------------
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_B': 64},  num_warps=2),
...
# Not currently used.
#
# Does not take advantage of block diagonal weight 
# matrices
#
...
# ---------------------------------------------------------------------------
# All scattering computation (direct and diffuse, left and right matrix blocks)
# together in a single kernel. Avoids some duplicated computation.
#
# Input data (mass/ke/w_eff/mu)
# loaded ONCE and shared.  Each MLP uses the BD v2 split-path approach
# (two independent [BLOCK_B, 16] streams).
# tau_pad kept live across both MLPs to avoid reloading.
#
# However, compared to USE_BD_V2_SCATTERING (combination of 
# _optical_split_direct_T4_bd_v2_kernel 
# and _optical_split_diffuse_T4_bd_v2_kernel), isn't faster 
# for T4 or L4 GPUs probably because of register
# spilling and oversubscribed L1 Cache. 
# 
# Run with USE_BD_V2_COMBINED_SCATTERING = True in evaluate_network_fast()
# ---------------------------------------------------------------------------
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_B': 64},  num_warps=2),
...
def launch_optical_scattering_fke(
        ...
        n_channels):
    """T4-optimized: fused optical depth (ke) + scattering.

    Does not take advantage of block diagonal structure for
    scattering
...
def launch_optical_scattering_combined(
    ...
    Does not take advantage of block diagonal
    weight matrices
...
def launch_optical_scattering_bd_v2(
    ...
    Splits into separate kernels for direct
    and diffuse radiation. Each kernel takes
    advantage of the block diagonal weight matrix.
    So far achieves the best performance for the 
    T4 GPU. Set as default in evaluate_network_fast.
    USE_BD_V2_SCATTERING = 1
...
def launch_optical_scattering_bd_v2_split(
        ...
        n_channels):
    """
    In addition to direct and diffuse split, splits each into separate kernels for the blocks in the block diagonal matrix.
    
    Gives a total for 4 kernels
    Goal was to reduce pressure on registers but incurs additional I/O to global memory that is very costly
    
    Corresponds to USE_BD_V2_SPLIT_SCATTERING in evaluate_network_fast.py
...
def launch_optical_scattering_combined_bd_v2(
    ...
    Puts all computation (direct and diffuse, left and right blocks) together in a single kernels. Avoids some duplicated computation.
    
    However, isn't faster for T4 GPU probably because of register spilling and oversubscribed L1 Cache. Also, slower on L4 GPU, too.
