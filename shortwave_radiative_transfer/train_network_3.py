...
train_network_3() - Trains neural network

Same as train_network.py except 
  (1) Allows all gases to influence every channel (removed filter variables)
  
Note that there is no train_network_2.py
...
class OpticalDepth(nn.Module):
    ...
    def __init__(self, n_channel, dropout_p, device):
        ...
        if False:
            n_weights_decomposition = 8 * n_channel  
            ...
            print(
                f"Gas decomposition actual number of weights = {n_weights_decomposition}")
    ...
    def forward(self, x):
        ...
        # tau_h2o : (n_examples, n_channels) * ((n_examples, 1) * (n_examples, 1))

        tau_h2o = a(self.net_h2o(one)) * (c[:, :, 2] * b(self.net_ke_h2o(t_p)))
                                                      
        tau_o3 = a(self.net_o3(one)) * (c[:, :, 3] * b(self.net_ke_o3(t_p)))
                                                      
        tau_co2 = a(self.net_co2(one)) * (c[:, :, 4] * b(self.net_ke_co2(t_p)))
                                                         
        tau_o2 = a(self.net_o2(one)) * (c[:, :, 5] * b(self.net_ke_o2(t_p)))
                                                       
        tau_n2o = a(self.net_n2o(one)) * (c[:, :, 6] * b(self.net_ke_n2o(t_p)))
                                                         
        tau_ch4 = a(self.net_ch4(one)) * (c[:, :, 7] * b(self.net_ke_ch4(t_p)))
